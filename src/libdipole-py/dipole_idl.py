...
def generate_ptr_class_code(interface_class):
    ...
    for member in inspect.getmembers(interface_class, predicate = inspect.isfunction):
        ...
        print(f"        await self.ws_handler.ws.send(json.dumps(message_json, cls = dipole.EnhancedJSONEncoder))", file = out)
    ...
    print(f"dipole.ptrs_map[backend_idl.{interface_class.__name__}] = {ptr_class_name}", file = out)
